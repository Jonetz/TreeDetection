...
def process_features(features, polygon_dict, id_to_area, containment_threshold, height_data, transform, width, height,
    ...
    heights, highest_points = get_height_within_polygon(polygon_x_gpu, polygon_y_gpu, height_data_gpu, transform, width,
                                                        height, bounds)
...
def update_feature_visualization(updated_features, contained_ids_per_feature, contained_ids_per_feature_flat,
                                 id_to_area, polygon_dict):
    """
...
def process_geojson(data, confidence_threshold, containment_threshold, height_data_path):
    ...
    confidence_scores = {feature['properties']['poly_id']: feature['properties']['Confidence_score'] for feature in
                         filtered_features}
    iou_threshold = 0.9
    area_threshold = 0.5
    # Apply filtering to keep only selected polygons
    retained_ids = filter_polygons_by_iou_and_area(polygon_dict, id_to_area, confidence_scores, iou_threshold,
                                                   area_threshold)
    ...
    filtered_features = process_features(filtered_features, polygon_dict, id_to_area, containment_threshold,
                                         height_data, transform, width_tif, height_tif, bounds)
    '''
...
def profile_code():
    ...
    pr.enable()
    process_files_in_directory(geojson_directory, height_directory, CONFIDENCE_THRESHOLD, CONTAINMENT_THRESHOLD,
                               parallel=False,
                               filename_pattern=("FDOP20_(\\d+)_(\\d+)_rgbi\\.tif", "nDSM_(\\d+)_1km\\.tif"))
